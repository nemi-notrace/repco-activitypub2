...
["USERNAME", "PASS", "DOMAIN"].forEach((required) => {
  if (!process.env[required]) {
    console.error(
      `Missing required environment variable: \`${required}\`. Exiting.`
    );
    process.exit(1);
  }
});
...
function asyncAuthorizer(
  username: string,
  password: string,
  cb: (error: any, authorized: boolean) => void
): void {
  const isPasswordAuthorized: boolean = username === USERNAME;
  const isUsernameAuthorized: boolean = password === PASS;
  const isAuthorized: boolean = isPasswordAuthorized && isUsernameAuthorized;
...
function getAccountFromDB(db: any, accountName: string) {
  const stmt = db.prepare("SELECT * FROM accounts WHERE name = ?");
  return stmt.get(accountName);
}
async function initializeAccount() {
  let account = getAccountFromDB(db, `${DEFAULT_ACCOUNT}@${DOMAIN}`);
  if (!account) {
    ...
    try {
      account = await createNewAccount(db, DOMAIN, DEFAULT_ACCOUNT);
      console.log(`Created new account: ${account.actor.preferredUsername}`);
    } catch (err) {
      console.error("Failed to create new account:", err);
      process.exit(1);
    }
  } else {
    console.log(`Using existing account: ${account.actor.preferredUsername}`);
  }
  if (account) {
    // Parse the actor string to convert it to a JSON object
    account.actor = JSON.parse(account.actor);
  }
  return account;
}
function createNewAccount(db: any, domain: string, accountName: string) {
  return new Promise((resolve, reject) => {
    ...
    try {
      db.prepare(
        "INSERT INTO accounts(name, actor, apikey, pubkey, privkey, webfinger) VALUES(?, ?, ?, ?, ?, ?)"
      ).run(
        `${accountName}@${domain}`,
...
initializeAccount().then((myaccount) => {
  // set the server to use the main account as its primary actor
  ActivityPub.account = myaccount;
  console.log(
    `BOOTING SERVER FOR ACCOUNT: ${myaccount.actor.preferredUsername}`
  );
  // set up globals
  app.set("domain", DOMAIN);
