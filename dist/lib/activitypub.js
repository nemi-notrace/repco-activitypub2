...
export class ActivityPubClient {
    ...
    async fetch(targetUrl, options) {
        ...
        const urlFragment = url.pathname +
    ...
    async send(recipient, message) {
        queue.enqueue(async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    console.error("Error sending outbound message:", response.status, response.statusText);
                }
                else {
